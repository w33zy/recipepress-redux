	...
	function get_the_rpr_taxonomy_headline( $taxonomy, $icons = false ) {
		...
		if ( $icons ) {
			...
			if ( $taxonomy === 'category' || $taxonomy === 'post_tag' ) {
				$icon_class = esc_attr( AdminPageFramework::getOption( 'rpr_options', array( 'tax_builtin',	$taxonomy, 'icon_class'	), 'fa-list-ul' ) );
			} else {
				$icon_class = esc_attr( AdminPageFramework::getOption( 'rpr_options', array( 'tax_custom', $optkey,	'icon_class' ), 'fa-list-ul' ) );
			}
...
if ( ! function_exists( 'get_the_rpr_taxonomy_terms' ) ) {
	 ...
	 * @param bool      $tags
	 *
	 * @return string $out rendered output
	 */
	function get_the_rpr_taxonomy_terms( $taxonomy, $icons = false, $label = false, $sep = '/', $tags = false ) {
		/**
		 *  Get the recipe id
		 */
		if ( isset( $GLOBALS['recipe_id'] ) && $GLOBALS['recipe_id'] !== '' ) {
		...
		if ( $tags && $terms && ! is_wp_error( $terms ) ) {
...
if ( ! function_exists( 'get_the_rpr_taxonomy_list' ) ) {
	 ...
	 * @param bool $label
	 * @param string $sep
	 * @param bool $tags set to true to enforce inclusion of terms (if active)
	 *
	 * @return string $out rendered output
	 */
	function get_the_rpr_taxonomy_list( $icons = false, $label = false, $sep = '/', $tags = false ) {
		/*
...
if ( ! function_exists( 'get_the_rpr_structured_data_header' ) ) {
	 ...
	 * three structured data types by http://schema.org is set in the options
	 *
	 * @since 0.8.0
	 * @return string
	 */
	function get_the_rpr_structured_data_header() {
		...
		if ( AdminPageFramework::getOption( 'rpr_options', array(
			...
			if ( (bool) AdminPageFramework::getOption( 'rpr_options', array(
					'metadata',
			...
			// Source
			if ( (bool) AdminPageFramework::getOption( 'rpr_options', array( 'metadata', 'use_source' ), false ) ) {
			...
			$out .= '}';
			$out .= '</script>';
		} else { // We will use our own structured data function
			$out = '';
		}
...
if ( ! function_exists( 'get_the_rpr_recipe_ingredients_headline' ) ) {
	...
	function get_the_rpr_recipe_ingredients_headline( $icons ) {
		 ...
		 */
		if ( $icons ) {
			$out .= '<i class="' . esc_attr( AdminPageFramework::getOption( 'rpr_options', array(
					'tax_builtin',
					'ingredients',
					'icon_class'
...
if ( ! function_exists( 'get_the_rpr_recipe_ingredients' ) ) {
	...
	function rpr_render_ingredient_grouptitle( $ingredient ) {
		...
		if ( recipe_is_embedded() ) {
			 ...
			 */
			$out .= '<h4 class="rpr-ingredient-group-title">' . sanitize_text_field( $ingredient['grouptitle'] ) . '</h4>';
		} else {
			/**
			 * Third level headline for standalone recipes
			 */
			$out .= '<h3 class="rpr-ingredient-group-title">' . sanitize_text_field( $ingredient['grouptitle'] ) . '</h3>';
		}
	...
	function rpr_render_ingredient_line( $ingredient ) {
		...
		$link_target = (int) AdminPageFramework::getOption( 'rpr_options', array( 'tax_builtin', 'ingredients',	'link_target' ), 2 );
		$microdata   = AdminPageFramework::getOption( 'rpr_options', array( 'metadata', 'structured_data_format' ), 'microdata' );
		$comment_sep = (int) AdminPageFramework::getOption( 'rpr_options', array( 'tax_builtin', 'ingredients',	'comment_sep' ), 0 );
		...
		if ( $microdata === 'microdata' ) {
			$out .= '<span itemprop="recipeIngredient" >';
		} elseif ( $microdata === 'rdfa' ) {
			$out .= '<span property="recipeIngredient" >';
		}

		// Render amount.
		if ( $ingredient['amount'] ) {
			$out .= '<span class="recipe-ingredient-quantity">' . sanitize_text_field( $ingredient['amount'] ) . '</span> ';
		}

		// Render the unit.
		if ( $ingredient['unit'] ) {
			$out .= '<span class="recipe-ingredient-unit">' . sanitize_text_field( $ingredient['unit'] ) . '</span> ';
		}
		...
		if ( $link_target === 0 ) {
			...
			// Set link to archive.
			$out         .= '<a href="' . get_term_link( $term->slug, 'rpr_ingredient' ) . '" rel="noopener">';
			$closing_tag = '</a>';
		} elseif ( $link_target === 2 ) {
			//  Set custom link if available, link to archive if not
			if ( isset( $ingredient['link'] ) && $ingredient['link'] !== '' ) {
				$out         .= '<a href="' . esc_url( $ingredient['link'] ) . '" target="_blank" rel="noopener">';
				$closing_tag = '</a> ';
			} else {
				$out .= '<a href="' . get_term_link( $term->slug, 'rpr_ingredient' ) . '" rel="noopener">';
			}
...
if ( ! function_exists( 'get_the_rpr_recipe_instructions' ) ) {
	...
	function rpr_render_instruction_block( $instruction ) {
		...
		if ( isset( $instruction['image'] ) && $instruction['image'] !== '' ) {
			...
			$instr_class .= ' ' . esc_attr( AdminPageFramework::getOption( 'rpr_options', array(
					'layout_general',
					'images_instr_pos'
				), 'right' ) );
		} else {
		...
		if ( isset( $instruction['image'] ) && $instruction['image'] !== '' ) {
			 ...
			 */
			$out .= '<img class="';
			$out .= esc_attr( AdminPageFramework::getOption( 'rpr_options', array(
				'layout_general',
				'images_instr_pos'
			), 'right' ) );
...
if ( ! function_exists( 'get_the_rpr_recipe_nutrition' ) ) {
	...
	function get_the_rpr_recipe_nutrition( $icons = false ) {
		 ...
		 * to be extended in the future to a complete set of nutritional data
		...
		if ( isset( $recipe['rpr_recipe_fat'][0] ) ) {
			$out .= sprintf( '<dt>' . __( 'Fat:', 'recipepress-reloaded' ) . '</dt><dd ' . $struct_fat . '>%s g</dd>', esc_html( $recipe['rpr_recipe_fat'][0] ) );
		}
		if ( isset( $recipe['rpr_recipe_protein'][0] ) ) {
...
if ( ! function_exists( 'get_the_alphabet_nav_bar' ) ) {
	function get_the_alphabet_nav_bar( $letters = false ) {
		...
		// Create an a empty output string
		$out = '';
		...
		foreach ( $alphabet as $a ) {
			...
			if ( $letters ) {
				if ( in_array( $a, (array) $letters, true ) ) {
...
/**
 * Replaces special characters and Umlaute with their basic letter
 *
